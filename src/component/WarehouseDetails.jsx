...
export default function WarehouseDetails() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const wRes = await axios.get("https://abhijitapi-1.onrender.com/api/warehouses");
      const selected = wRes.data.find((x) => x._id === id);
      setWarehouse(selected);

      const pRes = await axios.get(
        `https://abhijitapi-1.onrender.com/api/products/${id}`
      );
  ...
  const addProduct = async () => {
    ...
    await axios.post("https://abhijitapi-1.onrender.com/api/products", {
      warehouseId: id,
