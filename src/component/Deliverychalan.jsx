...
const [supplyType, setSupplyType] = useState("Outward");
  ...
  const saveChallan = async () => {
    await axios.post("https://aevix-chemical-3-gnuu.onrender.com/api/delivery-challan", form);
    setOpen(false);
    fetchData();
  };
const filteredData = data.filter((dc) => {
  ...
  const fetchData = async () => {
    const res = await axios.get("https://aevix-chemical-3-gnuu.onrender.com/api/delivery-challan");
    setData(res.data);
  };
