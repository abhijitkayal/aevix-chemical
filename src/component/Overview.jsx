...
//           axios.get('https://aevix-chem-backend-bksy.onrender.com/api/inward-payments'),
//           axios.get('https://aevix-chem-backend-bksy.onrender.com/api/outward-payments'),
//           axios.get('https://aevix-chem-backend-bksy.onrender.com/api/stocks')
//         ]);
...
const PaymentCard = ({ title, data }) => (
  <div className="bg-white rounded-xl shadow-md p-6">
    ...
    <div className="mt-4 space-y-2 text-sm">
      <div className="flex justify-between"><span>Cash</span><span>{formatAmount(data.cash)}</span></div>
      <div className="flex justify-between"><span>Online</span><span>{formatAmount(data.online)}</span></div>
      <div className="flex justify-between"><span>Cheque</span><span>{formatAmount(data.cheque)}</span></div>
      <div className="flex justify-between"><span>Bank</span><span>{formatAmount(data.bank)}</span></div>
    </div>
  </div>
);
...
const getTopAndLeastSelling = (sales = []) => {
  ...
  return {
    bestSelling: [...result].sort((a, b) => b.quantity - a.quantity).slice(0, 5),
    leastSelling: [...result].sort((a, b) => a.quantity - b.quantity).slice(0, 5)
  };
};
    ...
    const fetchAll = async () => {
      try {
        const [
          ...
          axios.get('https://aevix-chem-backend-bksy.onrender.com/api/outward-payments'),
          axios.get('https://aevix-chem-backend-bksy.onrender.com/api/stocks'),
          axios.get('https://aevix-chem-backend-bksy.onrender.com/api/sales')
        ]);
        ...
        const { bestSelling, leastSelling } =
          getTopAndLeastSelling(salesRes.data);
